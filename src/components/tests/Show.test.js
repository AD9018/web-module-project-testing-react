...
test("renders same number of options seasons are passed in", () => {
  render(<Show show={testShow} selectedSeason={"none"} />);
  const seasonMatch = screen.getAllByTestId("season-option");
  expect(seasonMatch).toHaveLength(4);
});

test("handleSelect is called when an season is selected", () => {
  const seasonFetch = jest.fn();
  render(
    <Show handleSelect={seasonFetch} show={testShow} selectedSeason={"none"} />
  );
...
test("component renders when no seasons are selected and when rerenders with a season passed in", () => {
  ...
  let episodeSelector = screen.queryAllByTestId("episodes-container");
  expect(episodeSelector).toHaveLength(0);

  rerender(<Show selectedSeason={1} show={testShow} />);
  episodeSelector = screen.getAllByTestId("episodes-container");
  expect(episodeSelector).toHaveLength(1);
});
